...
// ── Endpoint list — click to switch demo ─────────────────────────────────────
      ...
      {ENDPOINTS.map((ep) => {
        ...
        return (
          ...
          >
            <MethodBadge method={ep.method} />
            <code
              className={`font-mono text-sm ${isActive ? 'text-accent' : 'text-text-primary'}`}
            >
              {ep.path}
            </code>
            <span className="ml-2 hidden truncate text-xs text-text-secondary sm:block">
              {ep.description}
            </span>
            {ep.phase === 'v2' && (
              <span className="ml-auto shrink-0 rounded-full border border-border px-2 py-0.5 font-mono text-[8px] uppercase tracking-[0.12em] text-text-tertiary">
                v2
              </span>
            )}
...
export function ApiExplorer() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        <p className="mb-3 font-mono text-[10px] uppercase tracking-[0.24em] text-text-tertiary">
          All endpoints — click to explore
        </p>
...
p = pathlib.Path('/Users/ngoyuko/projects/sme-grant-navigator/src/components/ApiExplorer.tsx')
p.write_text(content)
lines = p.read_text().splitlines()
print(f'Written {len(lines)} lines. Last: {repr(lines[-1])}')
exports = [i+1 for i,l in enumerate(lines) if 'export function ApiExplorer' in l]
print(f'ApiExplorer exports at lines: {exports}')
